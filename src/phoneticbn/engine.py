    ...
    while i < word_len:
        ...
        if matched_key:
            ...
            if matched_key == 'y' and current_state == STATE_VOWEL:
                bengali_char = 'য়'
            if char_type == TYPE_MODIFIER: bengali_word += bengali_char
            elif char_type == TYPE_EXPLICIT_CONJUNCT:
                if current_state == STATE_CONSONANT: bengali_word = bengali_word[:-1]
                ...
                if current_state == STATE_CONSONANT: bengali_word += DIACRITICS.get(matched_key, '')
...
def post_process_word(bengali_word, original_english_word):
    if (original_english_word.endswith('o') and not original_english_word.endswith('O') and len(original_english_word) > 2):
        if original_english_word[-2] == 'y': pass
        else:
            VOWELS = "aeiouAEIOU"
            should_add_okar = False
            if len(original_english_word) == 2 and original_english_word[0] not in VOWELS: should_add_okar = True
            elif len(original_english_word) > 2:
                if original_english_word[-2] not in VOWELS and original_english_word[-3] in VOWELS: should_add_okar = True
